...
export default function EventList({ initialEvents, sortBy }: EventListProps) {
  ...
  const fetchMoreEvents = useCallback(async () => {
    if (loading || !hasMore) return; // Exit if already loading or no more events
    setLoading(true);
    try {
      const response = await fetch(`/api/events?page=${page}&sortBy=${sortBy}`);
      if (!response.ok) {
  ...
  }, [inView, fetchMoreEvents, hasMore, loading]);
